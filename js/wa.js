...
export function waButton(phone, message, label = 'WA') {
  const href = waHref(phone, message);
  if (!href) return '';
  return `<a href="${href}" target="_blank" rel="noopener" @click.stop class="inline-flex items-center gap-1 px-2.5 py-1.5 rounded-lg text-xs font-semibold text-white bg-[#25D366] hover:brightness-95 transition" title="Kirim pengingat via WhatsApp">${WA_ICON}${label}</a>`;
}
